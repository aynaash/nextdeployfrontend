...
import { 
  user, account, organization, team, teamMember, project, projectEnvironment,
  deployment, deploymentLog, metric, plan, subscription, billing, apiKey,
  webhook, webhookEvent, auditLog, twoFactor, passkey, session, verificationToken
} from './schema/schema';
...
async function seedDatabase() {
  ...
  const [johnDoe] = await db.insert(user).values({
    id: "usr_123456789",
    name: "John Doe",
  ...
  const [acmeInc] = await db.insert(organization).values({
    id: "org_123456789",
    name: "Acme Inc",
    slug: "acme-inc",
  ...
  const [frontendTeam] = await db.insert(team).values({
    id: "team_123456789",
  ...
  const [ecommerceProject] = await db.insert(project).values({
    id: "proj_123456789",
  ...
  const [prodEnv] = await db.insert(projectEnvironment).values({
    id: "env_123456789",
  ...
  const [ecommerceDeployment] = await db.insert(deployment).values({
    id: "dep_123456789",
    projectId: ecommerceProject.id,
  ...
  const [proPlan] = await db.insert(plan).values({
    id: "plan_123456789",
    name: "Pro Plan",
  ...
  const [acmeSubscription] = await db.insert(subscription).values({
    id: "sub_123456789",
    planId: proPlan.id,
    organizationId: acmeInc.id,
  ...
  const [deployWebhook] = await db.insert(webhook).values({
    id: "wh_123456789",
